# borrowed from https://github.com/val-iisc/SDAT/blob/main/common/utils/logger.py
import os
import sys
import time
import torch
...
class CompleteLogger:
    ...
    def __init__(self, root, phase='train'):
        if phase == 'debug':
            ...
            self.root = './logs/debug'
            log_filename = os.path.join(self.root, "{}.txt".format(phase))
        else:
            self.root = root
    ...
    def get_pickle_path(self, name=None):
        if name is None:
            name = self._get_phase_or_epoch()
        name = str(name)
        return os.path.join(self.visualize_directory, name + ".pkl")
